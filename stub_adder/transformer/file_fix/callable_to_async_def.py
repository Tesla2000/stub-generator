        ...
        for stmt in node.body:
            if (
                ...
                if isinstance(stmt.value, ast.Attribute):
                    ...
                    source = CallableToAsyncDef._find_method(
                        self._tree, ref_class, method_name
                    )
                ...
                if async_node is None:
                    ...
                    async_node = CallableToAsyncDef._build_async_def_from_expr(
...
class CallableToAsyncDef(ManualFix):
    ...
    _CALLABLE_ASSIGN_RE: ClassVar[re.Pattern[str]] = re.compile(
    ...
    def _parse_callable_params(
        ...
        if not inner:
            ...
            param_types = CallableToAsyncDef._split_top_level(inner)
        try:
            param_anns = [
    ...
    def _build_async_def_from_expr(
        ...
        parsed = CallableToAsyncDef._parse_callable_params(expr)
        if parsed is None:
            return None
    ...
    def is_applicable(self, errors: Iterable[str]) -> bool:
        return any(self._CALLABLE_ASSIGN_RE.search(e) for e in errors)
    ...
    def __call__(
        ...
        for error in errors:
            m = self._CALLABLE_ASSIGN_RE.search(error)
            if m:
