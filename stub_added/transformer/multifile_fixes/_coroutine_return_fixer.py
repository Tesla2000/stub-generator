...
_RETURN_INCOMPATIBLE_RE = re.compile(
...
def _parse_fixes(
    ...
    for errors in errors_by_file.values():
        for error in errors:
            m = _RETURN_INCOMPATIBLE_RE.search(error)
            if not m:
                continue
            coro_m = _COROUTINE_RE.search(m.group("full_type"))
            if not coro_m:
                continue
...
def _parse_assignment_fixes(
    ...
    for pyi, errors in errors_by_file.items():
        try:
            ...
            continue
        for error in errors:
            m = _ASSIGNMENT_RE.search(error)
            if not m:
                continue
            coro_m = _COROUTINE_RE.search(m.group("expr"))
            if not coro_m:
                continue
            lineno = int(m.group("line"))
            base_class = m.group("base_class")
            # Find the assignment target name at this line
            method = _assignment_target_at(tree, lineno)
            if method is None:
                continue
...
def _find_stub_for_module(module: str, reference_path: Path) -> Path | None:
        ...
        if isinstance(node, ast.ImportFrom) and node.module:
            for alias in node.names:
                if alias.name == class_name or alias.asname == class_name:
                    stub = _find_stub_for_module(node.module, pyi)
                    if stub is not None:
...
def _ensure_imports(tree: ast.Module, names: list[str], module: str) -> None:
...
class CoroutineReturnFixer(MultiFileFix):
    ...
    def __call__(
        ...
        fixes = _parse_fixes(errors_by_file)
        if fixes:
            ...
            for supertype_qualname, method_fixes in by_parent.items():
                ...
                parent_path = _find_stub_for_module(module, any_pyi)
        ...
        assignment_fixes = _parse_assignment_fixes(errors_by_file)
        for (pyi, base_class, method), coro in assignment_fixes.items():
            found = _find_class_module(base_class, pyi, stubs_dir)
            if found is None:
                continue
            parent_path, _ = found
            self._widen_parent(
    ...
    def _widen_parent(
        ...
        _ensure_imports(tree, ["Any", "Coroutine"], "typing")
        ...
        if upstream_errors:
            upstream_fixes = _parse_fixes(upstream_errors)
            ...
            for supertype_qualname, gp_method_fixes in by_grandparent.items():
                ...
                gp_path = _find_stub_for_module(gp_module, parent_path)
                if gp_path is None:
                    continue
                self._widen_parent(
